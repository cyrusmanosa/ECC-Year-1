...
public class Canvas {
    ...
    public void clear() {
        for (int y = 0; y < HEIGHT; y++) {
            for (int x = 0; x < WIDTH; x++) { pixel[y][x] = false; }
        }
        cx = 0;
        cy = 0;
    }
    ...
    public void show() {
        for (int y = 0; y < HEIGHT; y++) {
            for (int x = 0; x < WIDTH; x++) { System.out.print((pixel[y][x])? "○": "・"); }
            System.out.println();
        }
    }
    ...
    public void drawFont(boolean[][] fontdata) {
        ...
        for (int y = 0; y < fontdata.length; y++) {
            for (int x = 0; x < fontdata[y].length; x++ ) { setPoint(cx + x, cy + y, fontdata[y][x]); }
        }
        // カーソル進める
