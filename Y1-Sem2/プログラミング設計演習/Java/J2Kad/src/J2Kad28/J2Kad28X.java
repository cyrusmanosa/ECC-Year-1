...
class ECCVideo {
    ...
    private void notifySubscribers(VideoContent video) { for (var s : subscribers) {s.update(video);} }
    public void execute() {
        Scanner in = new Scanner(System.in);
...
// ビデオコンテンツ
class VideoContent { public String toString() { return "何かの動画コンテンツ"; } }
class DoraVideo extends VideoContent { public String toString() { return "ドラえもん"; } }
class PokeVideo extends VideoContent { public String toString() { return "ポケットモンスター"; }}
class AnpanVideo extends VideoContent { public String toString() { return "アンパンマン"; } }

// サブスクライバー
interface Subscriber { void update(VideoContent video); }
...
class Sizuka implements Subscriber {
    public void update(VideoContent video) {
        if (video instanceof DoraVideo) {System.out.println("しずか：これ一番見たかったの！"); }
        else { System.out.println("しずか：どれも楽しいわ！"); }
    }
    public String toString() { return "しずか"; }
}
class Suneo implements Subscriber {
    public void update(VideoContent video) {
        if (video instanceof AnpanVideo) { System.out.println("スネ夫：実はドキンちゃん、好きなんだ！"); }
        else { System.out.println("スネ夫：いや～どれも良くできてるね～！"); }
    }
    public String toString() { return "スネ夫"; }
}
class Jaian implements Subscriber {
    public void update(VideoContent video) {
        if (video instanceof PokeVideo) { System.out.println("ジャイアン：ピカチュウ！心の友よ！！"); }
        else { System.out.println("ジャイアン：時間がたつのを忘れるぜ！"); }
    }
